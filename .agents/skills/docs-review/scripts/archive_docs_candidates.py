#!/usr/bin/env python3
"""
Move clear archive candidates from docs/ into an archive folder.
"""
...
from docs_cleaner import ARCHIVE, classify_file, extract_canonical_keep_set


def archive_candidates(docs_root: Path, archive_root: Path, apply: bool) -> list[tuple[Path, Path]]:
    canonical_keep = extract_canonical_keep_set(docs_root)
    manual_review_root = docs_root / "manual_review_needed"
...
def main() -> int:
    ...
    candidates = archive_candidates(docs_root, archive_root, apply=args.apply)
    print(f"archive_candidates={len(candidates)}")
    return 0
